...
class PlumeDroneBulletEnv(BaseRLAviary):
    def __init__(
        ...
        # TODO: add move_incrementer as an init parameter instead of manually setting
        self._action_to_direction = {
            0: np.array([0, 0.4, 0]),  # right
            1: np.array([0.4, 0.4, 0]),  # down-right
            2: np.array([0.4, 0, 0]),  # down
            3: np.array([0.4, -0.4, 0]),  # down-left
            4: np.array([0, -0.4, 0]),  # left
            5: np.array([-0.4, -0.4, 0]),  # up-left
            6: np.array([-0.4, 0, 0]),  # up
            7: np.array([-0.4, 0.4, 0]),  # up-right
        }

        # initializing concentration map, which will be observation space
        self.concentrations = np.full((self.size, self.size), self.background_concentration, dtype=float)
    ...
    def reset(self, seed=None):
        ...
        if seed is not None:
            np.random.seed(seed)
        self.concentrations = np.full((self.size, self.size), self.background_concentration, dtype=np.float32)
    ...
    def _computeObs(self):
        ...
        state = {
            ...
            "concentrations": np.array(self.get_concentrations())
        }
        return state
    ...
    def _observationSpace(self):
        ...
        return spaces.Dict({
            ...
            "concentrations": spaces.Box(low=0, high=100, shape=(self.size, self.size), dtype=np.float32)
        })
    ...
    def get_concentrations(self):
        ...
        for drone_position in positions:
            ...
            # Apply Gaussian filter and update the concentrations map. Only take x,y since z values don't matter here
            x, y = int(drone_position[0]), int(drone_position[1])  # Assuming positions are 2D
            self.concentrations[x][y] = self.gaussian(shortest_distance)
